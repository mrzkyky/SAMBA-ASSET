  ...
  const previewItems = useMemo(() => {
    ...
    const individualItems = parsedRawRows.map((row, idx) => {
      ...
      if (rawSite && rawSite.trim()) {
        ...
        rawSite = lastKnownSite; // Inherit site name from upper merged cell!
      }
      ...
      let detectedModel = rawModel || (rawCategory && rawCategory !== detectedCat ? rawCategory : (detectedCat !== 'Umum / Lainnya' ? `${detectedCat} Standar` : 'Unit Standar'));
      ...
      // 4. Serial Number Auto-Healing & Multi-line Model/SN Disentanglement
      let rawSN = getVal('serial_number');

      // Handle multi-line cells where Line 1 is Model (e.g. DS-2CD1327G2H-LIUF2) and Line 2 is SN (e.g. 0231129AAWRAX0611053)
      if (rawSN && (rawSN.includes('\n') || rawSN.includes('\r'))) {
        const snLines = rawSN.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
        if (snLines.length >= 2 && (!rawModel || rawModel.includes('HIKVISION') || rawModel === 'Unit Standar')) {
          detectedModel = snLines[0];
          rawSN = snLines.slice(1).join(', ');
        }
      }
